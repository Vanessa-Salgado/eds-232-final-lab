...
```
```{r}
xgb_best = show_best(xgb_cv_tune, n = 1, metric = "roc_auc") #get metrics for best boosted regression
xgb_best
```
...
#testing prediction
test_predict_xgb = predict(train_fit_xgb, test) %>% #get testing prediction
  bind_cols(test)  #bind to testing column

test_predict2_xgb = predict(train_fit_xgb, test, type = "prob") %>% #get prediction probabilities for test 
  bind_cols(test) #bind to testing column



#testing accuracy
accuracy(test_predict_xgb, truth = dic, estimate = .pred_class) #get accuracy of testing prediction

test_roc_auc_xgb = roc_curve(test_predict2_xgb, dic, .pred_bathydemersal, .pred_bathypelagic, .pred_benthopelagic, .pred_demersal, .pred_pelagic, `.pred_pelagic-neritic`, `.pred_pelagic-oceanic`, `.pred_reef-associated`)
